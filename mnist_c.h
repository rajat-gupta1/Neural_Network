...
void image_char2double2(int num_data, unsigned char **data_label_char, double *data_label)
{
    int i;
    for (i=0; i<num_data; i++)
    {
        // printf("%u ", data_label_char[i][0]);
        data_label[i]  = (int)data_label_char[i][0];
        // printf("%f ", data_label[i]);
    }
}
...
void load_mnist()
{
    ...
    train_image_char = (unsigned char**)malloc(NUM_TRAIN * sizeof(unsigned char*));
    test_image_char = (unsigned char**)malloc(NUM_TEST * sizeof(unsigned char*));
    train_label_char = (unsigned char**)malloc(NUM_TRAIN * sizeof(unsigned char*));
    test_label_char = (unsigned char**)malloc(NUM_TRAIN * sizeof(unsigned char*));

    for (int i = 0; i < NUM_TRAIN; i++)
    {
        train_image[i] = (double*)malloc(SIZE * sizeof(double));
        train_image_char[i] = (unsigned char*)malloc(SIZE * sizeof(unsigned char));
        train_label_char[i] = (unsigned char*)malloc(1 * sizeof(unsigned char));
    }

    for (int i = 0; i < NUM_TEST; i++)
    {
        test_image[i] = (double*)malloc(SIZE * sizeof(double));
        test_image_char[i] = (unsigned char*)malloc(SIZE * sizeof(unsigned char));
        test_label_char[i] = (unsigned char*)malloc(1 * sizeof(unsigned char));
    }
...
void print_mnist_label(int data_label[], int num_data)
{
    ...
    if (num_data == NUM_TRAIN)
        for (i=0; i<num_data; i++)
            printf("train_label[%d]: %f\n", i, train_label[i]);
    else
        for (i=0; i<num_data; i++)
            printf("test_label[%d]: %f\n", i, test_label[i]);
}
